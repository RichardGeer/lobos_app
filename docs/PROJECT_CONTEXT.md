...
Here is a **clean summary block you can paste directly into `PROJECT_CONTEXT.md`** to start the next thread.

I included a **Pacific Time timestamp** so ordering stays clear.

---

# Development Log

## 2026-03-14 18:45 PST — Recipe Rendering + Quality Model Fix

### UI / Rendering Improvements

Implemented a **model-agnostic recipe markdown normalization layer** in `my_recipe.html`.

Problem:

* LLM responses often returned **label-style text** rather than proper markdown.
* Example:
...
This produced **flat rendering** even when using `marked.js`.

Solution:
Added a client-side preprocessing step:

```
normalizeRecipeMarkdown()
```

This converts label patterns into real markdown before rendering.
...
→ store cleaned markdown text in DB
→ browser loads text
→ normalizeRecipeMarkdown()
→ marked.parse()
→ DOMPurify.sanitize()
...
Added server-side cleanup to strip markdown fences before saving recipe text.

This prevents markdown renderers from displaying code blocks.

---

### Quality Model Mode Fix

Problem:
Quality mode appeared broken.

Logs showed:

```
recipe_generate_request user=8 quality=1 want_quality=True
recipe_user_cache_hit user=8 recipe_id=107
```

Root cause:
Cache lookup did **not include model identity**, so:

```
fast recipe cache
and
quality recipe cache
shared the same bucket
```

Quality requests were returning **fast-model cached recipes**.

Solution:
Cache functions updated to include `model` filtering.

Updated functions in `recipe_service.py`:

```
get_user_cached_recipe()
find_shared_recipe_candidates()
get_cached_recipe()
get_or_clone_shared_recipe()
```
...
```
FAST mode
→ cache only FAST_MODEL results

QUALITY mode
→ cache only QUALITY_MODEL results
```

Example correct log behavior:

```
recipe_generate_request user=8 quality=1 want_quality=True
recipe_generate user=8 model=llama3.1:8b-instruct-q8_0
```

or

```
recipe_user_cache_hit user=8 recipe_id=123 model=llama3.1:8b-instruct-q8_0
```
...
✔ recipe generation
✔ fast vs quality model switching
✔ cache reuse
✔ force_new bypass
✔ recipe history
...
* much cleaner recipe presentation
...
Candidates for next development thread:

1️⃣ **Recipe vector similarity filtering**

Avoid storing near-duplicate recipes.

Possible approach:

```
recipe_embedding vector
cosine_similarity threshold
```

2️⃣ **Global recipe pool**

Allow users to reuse recipes generated by other users with similar overlays.

3️⃣ **Background recipe pre-generation worker**
...
generate N recipes
store in pool
serve instantly later
```

4️⃣ **Recipe variant system**
...
to prevent variant explosion.

---

If you'd like, next thread I can also help you implement what is probably the **single biggest performance upgrade for Lobos**:

**background recipe pre-generation + shared recipe pool**

This can reduce AI calls by **90-99%** once the system scales.
